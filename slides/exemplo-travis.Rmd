...
Para cada build job, consideramos o número de testes executados, reportado no campo `tr_tests_run` do data set...
...
**RQ1. Quantos testes são executados por build em projetos que adotam integração contínua?** A Figura 1 apresenta um histograma do número de testes executados por build. Como esperado, o histograma segue uma distribuição próxima à distribuição power-law frequentemente observada em estudos empíricos: a maioria dos projetos executa uma quantidade menor de testes, enquanto alguns executam muitos testes (média: `r mean(builds$tr_tests_run, na.rm=T)`; mediana: `r median(builds$tr_tests_run, na.rm=T)`; 95%: `r quantile(builds$tr_tests_run, 0.95, na.rm=T)`).
